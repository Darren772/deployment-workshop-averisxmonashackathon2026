...
There is no database and no external API. `data/places.ts` is the "database".
...
/dashboard ("use client", can't see secrets)
    → fetch("/api/places")
        → route.ts reads MAKAN_API_SECRET (server-only)
            → data/places.ts
```

Never "simplify" `/dashboard` by importing `data/places.ts` directly — that would delete the lesson.
...
`MAKAN_API_SECRET` and `ADMIN_PASSWORD` are server-only; `NEXT_PUBLIC_APP_NAME` and `NEXT_PUBLIC_API_VERSION` are public. `VERCEL_ENV` and `VERCEL_GIT_COMMIT_SHA` are injected by Vercel and fall back to `"local"`.
